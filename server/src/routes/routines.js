...
router.get('/routines', (req, res) => {
  ...
  const routines = db
    .prepare('SELECT id, name FROM routines WHERE user_id = ? ORDER BY name')
    .all(req.userId);
...
router.get('/routines/:id', (req, res) => {
  ...
  // routine IDs exist.
  const routine = db
    .prepare('SELECT id, name FROM routines WHERE id = ? AND user_id = ?')
    .get(routineId, req.userId);
  if (!routine) {
    return res.status(404).json({ error: 'routine not found' });
  }
...
router.post('/routines/:id/exercises', (req, res) => {
  ...
  const info = db
    ...
    .run(routineId, exercise_id, target_sets ?? null, target_reps ?? null);
